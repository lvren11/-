...
def direct_match(project,storage,para,min_length,plan,sym,idlist,rollindex,prolist,record,step):#宽度直接匹配
    ...
    for item in project:#对每一个主项目需求
        ...
        froll=[]#分配完的母卷
        storage.sort(key=lambda x:(x.width,x.length,str(x.id)),reverse=True)#母卷按照宽度、长度和id号降序排序
...
def used_cut(sorted_storage,project,roll,curroll,froll,fneed,maxlist,maxsum,para,min_length,plan,remain_min,sym,length_difference,prolist,record,step):
    ...
    if need_satisfy(shortneed.length,roll.length*num,shortneed.left,shortneed.right,min_length)==True:#母卷全分配
        ...
        if flag==1:#所有需求满足要求
            ...
            if used_roll_satisfy(curroll,templist,roll.length,shortneed,tempdic,min_length)==True:
                content={}
                content['width']=[]
                content['length']=roll.length
                content['symbol']=[]
...
def width_match(project,storage,para,min_length,plan,remain_min,sym,method,length_difference,prolist,idlist,rollindex,sorted_storage,left_cut,record,step):#基于宽度组合匹配
    storage.sort(key=lambda x:(x.widthscale.right,x.length,str(x.id)),reverse=True)#母卷按照宽度、长度、id降序排序
    ...
    for roll in storage:#遍历母卷
        ...
        while 1:#母卷一直做宽度匹配，直到用完或没有合适的宽度组合
            ...
            if left==1 and roll.id not in sorted_storage:#如果上一个母卷最后一个组合没分配完，且当前母卷和其他母卷没有连续关系
                ...
                while 1:
                    for subset in combinations_with_replacement(project,i):#宽度所有组合
                        ...
                        if tempsum>maxsum and inscale(tempsum,roll.widthscale) and found==0:#利用率更大且落在母卷宽度范围内
                            #for x in subset:
                                #print(x.width,end=' ')
                            #print()
                            maxsum=tempsum
    ...
    return data,total,rollindex,left_cut
...
def length_match(project,storage,para,min_length,plan,remain_min,sym,method,otherproject,prolist,idlist,rollindex,record,step):#基于长度组合分配
    ...
    while 1:
        ...
        storage.sort(key=lambda x:(x.widthscale.right,x.length,str(x.id)),reverse=True)#母卷按长度从大到小
...
def get_combination(project,cut_length,max_width,min_length,method):#已知切割长度和剩余宽度，获取最优宽度组合
...
def quick_match(roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step):
    ...
    maxsum,maxlist=get_combination(tempproject,content['length'],roll.widthscale.right-curwidth,min_length,method)
    ...
    for index in range(len(otherproject)):
        ...
        tempproject=[x for x in otherproject[index] if x not in fneed]
        maxsum3,maxlist=get_combination(tempproject,content['length'],roll.widthscale.right-curwidth-maxsum-maxsum2,min_length,method)
        maxsum2=maxsum2+maxsum3
    ...
    remainlist=remain_process(roll.widthscale.right-curwidth-maxsum-maxsum2,remain_min)#分配余材
    for width in remainlist:
        content['width'].append(width)
        content['symbol'].append(sym['余材'])
    ...
    return content,total1,total2,rollindex

def quick_match_2(roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step):
    ...
    remainlist=remain_process(roll.widthscale.right-curwidth-maxsum-maxsum2,remain_min)
    for width in remainlist:
        content['width'].append(width)
        content['symbol'].append(sym['余材'])
    ...
    return flag,content,total1,total2,rollindex

def quick_match_3(roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step):
...
def cut(lastroll,roll,content_width,content_sym,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step):
    ...
    if need_satisfy(shortneed.length,roll.length*num,shortneed.left,shortneed.right,min_length):#母卷能全分配满足需求
        ...
        if tempflag==0:
            ...
            return isleft,total1,total2,rollindex
...
def all_match(mainproject,otherproject,storage,prolist,allproject,para,min_length,plan,remain_min,sym,method,idlist,rollindex,sorted_storage,left_cut,record,step):#最后一步分配完所有主需求
    ...
    for mainneed in mainproject:#对于每一个主需求
        ...
        storage.sort(key=lambda x:(x.widthscale.right,x.length,str(x.id)),reverse=True)#母卷按宽度从大到小
            ...
            while 1:
                ...
                if isleft==1 and roll.id not in sorted_storage:#有剩余组合，且当前母卷无连续关系
                    ...
                    if inscale(tempsum,roll.widthscale) or (roll.widthscale.left>tempsum and roll.widthscale.left-tempsum<remain_min):
                        isleft,d1,d2,rollindex=cut(lastroll,roll,leftneed,leftsym,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step)
                        total1=total1+d1
                        total2=total2+d2 
                        #注意：母卷要么分配完，要么满足需求组合
                ...
                #母卷全分配 and 满足需求    
                result,d1,d2,rollindex=quick_match(roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step)
                total1=total1+d1
                total2=total2+d2
                ...
                if need_satisfy(mainneed.length,roll.length,mainneed.left,mainneed.right,min_length)==True:#母卷全分配
                    ...
                    tempproject=[x for x in mainproject if x not in fneed]
                    maxsum,maxlist=get_combination(tempproject,roll.length,roll.widthscale.right-mainneed.width,min_length,method)
                    for x in maxlist:
                        content['width'].append(x.width)
                        content['symbol'].append(sym['项目'][0])
                    ...
                    for index in range(len(otherproject)):
                        ...
                        tempproject=[x for x in otherproject[index] if x not in fneed]
                        maxsum3,maxlist=get_combination(tempproject,content['length'],roll.widthscale.right-mainneed.width-maxsum-maxsum2,min_length,method)
                        maxsum2=maxsum2+maxsum3
                    ...
                    remainlist=remain_process(roll.widthscale.right-mainneed.width-maxsum-maxsum2,remain_min)
                    for width in remainlist:
                        content['width'].append(width)
                        content['symbol'].append(sym['余材'])
                ...
                if roll_satisfy(roll.length,mainneed.length,min_length)==True:#母卷可以恰好满足需求，需求完成，母卷可能有剩余
                    ...
                    tempproject=[x for x in mainproject if x not in fneed]
                    maxsum,maxlist=get_combination(tempproject,mainneed.length,roll.widthscale.right-mainneed.width,min_length,method)
                    for x in maxlist:
                        content['width'].append(x.width)
                        content['symbol'].append(sym['项目'][0])
                    ...
                    for index in range(len(otherproject)):
                        ...
                        maxsum3,maxlist=get_combination(tempproject,mainneed.length,roll.widthscale.right-mainneed.width-maxsum-maxsum2,min_length,method)
                        maxsum2=maxsum2+maxsum3
                ...
                #for i in range(int(max(mainneed.lengthscale.left,min_length)),int(mainneed.lengthscale.right)):#满足需求溢短差，需求完成，母卷可能有剩余
                    if roll_satisfy(roll.length,i,min_length)==True:
                        ...
                        tempproject=[x for x in mainproject if x not in fneed]
                        maxsum,maxlist=get_combination(tempproject,content['length'],roll.widthscale.right-mainneed.width,min_length,method)
                        for x in maxlist:
                            content['width'].append(x.width)
                            content['symbol'].append(sym['项目'][0])
                ...
                #母卷全分配，需求不一定满足
                tempflag,result2,d1,d2,rollindex=quick_match_2(roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step)
                ...
                if result2!={}:#成功分配
                    ...
                    result3,d1,d2,rollindex=quick_match_3(roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step)
                    total1=total1+d1
                    total2=total2+d2
