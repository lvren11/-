...
class Ui_Dialog_UP(QDialog):
    def __init__(self, parent=None):
        super(Ui_Dialog_UP, self).__init__(parent)
        self.parent = parent
    ...
    def load_ui(self):
        if self.parent.selected_item_text != None:
            self.lineEdit.setText(self.parent.selected_item_text)
            self.lineEdit.setFocusPolicy(Qt.NoFocus)
        else:
            self.box = QMessageBox.critical(self, '错误', '没有双击选中修改密码的帐号',QMessageBox.Yes, QMessageBox.No)
        ...
        for i in range(len(self.parent.user)):
            if self.parent.user[i]['account'] == str(self.lineEdit.text()):
                if self.lineEdit_2.text() == self.lineEdit_3.text():
                    self.parent.user[i]['password'] = md.hexdigest()
                    self.close()
                else:
