...
class Need():
    ...
    def __init__(self,width,weight,para,no_limit,index):
...
def get_project(filename,pos,neg,Need_encoding,para):#获取需求项目规格
    ...
    for item in tempfile:#遍历文件每一行
        ...
        if item[0]=='宽度' or item[0]=='宽左':#标题栏
            ...
            if mark==0:#新需求
                need=Need(item[0],item[1],para,item[2],project_index)
                if neg>0 or pos<0:
...
def need_satisfy(root_length,cut_length,left_length,right_length,min_length,no_limit):#判断需求切割后是否恰好满足or满足最短米数or满足溢短差
    length_scale=Interval(root_length+left_length,root_length+right_length)

    if no_limit==1:
        if root_length-cut_length>=min_length or cut_length>=root_length:
            return True
        else:
            return False
    ...
    for item in project:#对每一个主项目需求
        ...
        for roll in storage:#对于每一个母卷
            ...
            if inscale(item.width,roll.widthscale):#需求宽度在母卷宽度范围内
                ...
                if need_satisfy(item.length,roll.length,item.left,item.right,min_length,item.no_limit)==True:#母卷全分配
                    print("step 2:")
                    content={}
                    content['width']=[item.width,]
...
def used_roll_satisfy(sorted_storage,roll,templist,cut_length,shortneed,tempdic,min_length):
    ...
    if int((shortneed.length-cut_length)/tempdic[str(shortneed.width)+str(shortneed.index)]+0.5)<min_length:#下一卷的切割长度不能小于最短米数
        return False

    flag=0
    for need in templist:#下一个母卷全分配
        if need_satisfy(need.length-cut_length*tempdic[str(need.width)+str(need.index)],roll.length*tempdic[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
            flag=1
            break
    if flag==0:
        return True
    ...
    flag=0
    cut_length_next=int((shortneed.length-cut_length)/tempdic[str(shortneed.width)+str(shortneed.index)]+0.5)
    for need in templist:
        if need_satisfy(need.length-cut_length*tempdic[str(need.width)+str(need.index)],tempdic[str(need.width)+str(need.index)]*cut_length_next,need.left,need.right,min_length,need.no_limit)==False:
            flag=1
            break
    if flag==0:
        return True
    ...
    interval.left=interval.left/tempdic[str(shortneed.width)+str(shortneed.index)]-cut_length#更新米数范围
    interval.right=interval.right/tempdic[str(shortneed.width)+str(shortneed.index)]-cut_length
    ...
    for i in range(int(max(min_length,interval.left)),int(interval.right)):#满足最短需求溢短差
        if roll_satisfy(roll.length,i,min_length):#母卷满足要求
            ...
            for one in templist:
                if need_satisfy(one.length,tempdic[str(one.width)+str(one.index)]*i,one.left,one.right,min_length,one.no_limit)==False:#母卷不满足
                    flag=0
                    break
...
def used_cut(sorted_storage,project,roll,curroll,froll,fneed,maxlist,maxsum,para,min_length,plan,remain_min,sym,length_difference,prolist,record,step):
    ...
    for one in maxlist:#初始化tempdic和templist
        if one not in templist:
            templist.append(one)
            tempdic[str(one.width)+str(one.index)]=0

    for one in maxlist:#计算组合内需求出现的次数
        tempdic[str(one.width)+str(one.index)]=tempdic[str(one.width)+str(one.index)]+1
    ...
    if limit_flag==0:
        ...
        for tempitem in maxlist:
            if tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<shortneed.length/tempdic[str(shortneed.width)+str(shortneed.index)]:
                shortneed=tempitem
    
    else:

        for tempitem in maxlist:
            if tempitem.no_limit==0 and tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<shortneed.length/tempdic[str(shortneed.width)+str(shortneed.index)]:
                shortneed=tempitem

        

    interval=copy.deepcopy(shortneed.lengthscale)#获取最短需求的米数范围
    num=tempdic[str(shortneed.width)+str(shortneed.index)]#最短需求出现的次数
    ...
    for one in maxlist:#需求间长度差>=指定值,对于组合内每个需求，如果不是准确切割 and 不是满足溢短差 and 切割后长度<指定值
        ...
        if one.no_limit==1 and tempdic[str(one.width)+str(one.index)]*cut_length>=one.length:
            continue
        if inscale(tempdic[str(one.width)+str(one.index)]*cut_length,one.lengthscale)==False and one.length-tempdic[str(one.width)+str(one.index)]*cut_length<length_difference:
            #print(one.width)
            tempflag=1
            break
    ...
    if need_satisfy(shortneed.length,roll.length*num,shortneed.left,shortneed.right,min_length,shortneed.no_limit)==True:#母卷全分配
        print("used step 1:")
        flag=1#标识组合是否满足母卷条件

        for one in templist:
            if need_satisfy(one.length,tempdic[str(one.width)+str(one.index)]*roll.length,one.left,one.right,min_length,shortneed.no_limit)==False:#母卷不满足
                flag=0
                break
        ...
        if flag==1:#所有需求满足要求
            ...
            if used_roll_satisfy(sorted_storage,curroll,templist,roll.length,shortneed,tempdic,min_length)==True:
                ...
                if done==0:
                    print("#########return 4")
                    return content,0,total#没分配完，母卷用完了
                else:
                    print("#########return 5")
                    return content,1,total#分配完，母卷也用完
    ...
    if roll_satisfy(roll.length,cut_length,min_length)==True:#最短需求满足
        ...
        for one in templist:
            if need_satisfy(one.length,tempdic[str(one.width)+str(one.index)]*cut_length,one.left,one.right,min_length,one.no_limit)==False:#母卷不满足         
                flag=0
                break
        ...
        if flag==1:#所有需求满足要求
            ...
            if used==0:
                ...
                print("#########return 7")
                return content,2,total#分配完，母卷没用完
...
def get_better_roll(rolllist,leftneed,min_length,length_difference):
    ...
    for one in leftneed:#初始化tempdic和templist
        if one not in templist:
            templist.append(one)
            tempdic[str(one.width)+str(one.index)]=0

    for one in leftneed:#计算组合内需求出现的次数
        tempdic[str(one.width)+str(one.index)]=tempdic[str(one.width)+str(one.index)]+1
    ...
    if limit_flag==0:
        ...
        shortneed=leftneed[0]#找出切割长度最短的需求
        for tempitem in leftneed:
            if tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<shortneed.length/tempdic[str(shortneed.width)+str(shortneed.index)]:
                shortneed=tempitem
    ...
    for one in leftneed:#需求间长度差>=指定值,对于组合内每个需求，如果不是准确切割 and 不是满足溢短差 and 切割后长度<指定值
    ...
    for index in range(len(rolllist)):
        ...
        if need_satisfy(shortneed.length,roll.length*num,shortneed.left,shortneed.right,min_length,shortneed.no_limit)==True:#母卷全分配
            ...
            for one in templist:
                if need_satisfy(one.length,tempdic[str(one.width)+str(one.index)]*roll.length,one.left,one.right,min_length,one.no_limit)==False:#母卷不满足
                    flag=0
                    break
...
def width_match(project,otherproject,storage,para,min_length,plan,remain_min,sym,method,length_difference,prolist,idlist,rollindex,sorted_storage,left_cut,record,step):#基于宽度组合匹配
    ...
    for j in range(len(storage)):
        ...
        if left==1:
            ...
            if inscale(tempsum,roll.widthscale)==True:
                print("找最优卷")
                temprolllist=[x for x in storage if x not in froll]
                roll_same_width=[]
                for x in temprolllist:
                    if x.widthscale.right==roll.widthscale.right:
                        roll_same_width.append(x)

                better_index=get_better_roll(roll_same_width,leftneed,min_length,length_difference)

                print("better_index:"+str(better_index))

                if better_index!=-1:
                    index1=storage.index(roll_same_width[better_index])
                    storage[index1],storage[j]=storage[j],storage[index1]
    ...
    #for roll in storage:#遍历母卷
        ...
        while 1:#母卷一直做宽度匹配，直到用完或没有合适的宽度组合
            ...
            tempnum=int(roll.widthscale.right/project[-1].width)
            if tempnum<=3:
                maxnum=tempnum
                        ...
                        if inscale(tempsum,roll.widthscale):#判断母卷是不是能全部分配
                            ...
                            if limit_flag==0:
                                ...
                                for tempitem in subset:
                                    if tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<tempshortneed.length/tempdic[str(tempshortneed.width)+str(tempshortneed.index)]:
                                        tempshortneed=tempitem
                            
                            else:

                                for tempitem in subset:
                                    if tempitem.no_limit==0 and tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<tempshortneed.length/tempdic[str(tempshortneed.width)+str(tempshortneed.index)]:
                                        tempshortneed=tempitem

                            length_scale=Interval((tempshortneed.length+tempshortneed.left)/tempdic[str(tempshortneed.width)+str(tempshortneed.index)],(tempshortneed.length+tempshortneed.right)/tempdic[str(tempshortneed.width)+str(tempshortneed.index)])
            ...
            if used==1 and maxsum!=0 and roll.id not in sorted_storage:#上一卷没用完
                tempcontent,response,d=used_cut(sorted_storage,project,usedroll,roll,froll,fneed,maxlist,maxsum,para,min_length,plan,remain_min,sym,length_difference,prolist,record,step)
                ...
                if response==-1 or response==2:#上一卷不可分配 or 分配完，母卷没用完
                    ...
                    if tempcontent!={} and tempcontent['width']==lastwidth and last_roll.id==usedroll.id:
                        left_cut=left_cut+1
                ...
                if roll_satisfy(roll.length,cut_length,min_length)==True:#最短需求满足
                    ...
                    if flag==1:#所有需求满足要求
                        ...
                        if left==1:
                            ...
                            if content!={} and lastwidth==content['width'] and roll.id==last_roll.id:
                                left_cut=left_cut+1
        ...
        if len(relation)!=0:#存在倍数组合
            ...
            if maxsum!=0:#有最优宽度组合
                ...
                for one in tempproject:
                    if one.width<=new_widthscale.right and need_satisfy(one.length,maxroll[1],one.left,one.right,min_length,one.no_limit):#满足长度宽度要求的剩余需求
                        itemlist.append(one)
                ...
                while 1:#完成剩余宽度的组合分配
                    for subset in combinations_with_replacement(itemlist,i):#宽度所有组合
                        ...
                        for one in templist:
                            if need_satisfy(one.length,tempdic[str(one.width)+str(one.index)]*maxroll[1],one.left,one.right,min_length,one.no_limit)==False:
                                flag=0
                                break
    ...
    while 1:
        ...
        for subset in combinations_with_replacement(project,i):
            ...
            if tempsum<=max_width and tempsum>maxsum:
                ...
                for one in templist:
                    if need_satisfy(one.length,tempdic[str(one.width)+str(one.index)]*cut_length,one.left,one.right,min_length,one.no_limit)==False:
                        flag=0
                        break
...
def quick_match_3(need_not_done,need_same_width,lastwidth,roll,mainproject,otherproject,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step):
    ...
    for mainneed in tempproject:
        ...
        for p in range(1,maxnum+1):#从1~maxnum个
            ...
            if roll_satisfy(roll.length,mainneed.length/p,min_length)==True and mainneed.length/p>=min_length:#需求恰好被满足
...
def cut(lastroll,roll,content_width,content_sym,sym,prolist,para,fneed,min_length,method,allproject,remain_min,plan,froll,idlist,rollindex,record,step):
    ...
    for one in otherneed:#初始化tempdic和templist
        if one not in templist2:
            ...
            tempdic2[str(one.width)+str(one.index)]=0

    for one in otherneed:#计算组合内需求出现的次数
        tempdic2[str(one.width)+str(one.index)]=tempdic2[str(one.width)+str(one.index)]+1
    ...
    for one in allneed:
        if one.no_limit==0:
            shortneed=one
            if one in mainneed:
                shortnum=tempdic[str(one.width)+str(one.index)]
            else:
                shortnum=tempdic2[str(one.width)+str(one.index)]
            limit_flag=1
            break
    ...
    if limit_flag==0:
        ...
        shortnum=tempdic[str(shortneed.width)+str(shortneed.index)]
        for tempitem in mainneed:
            if tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<shortneed.length/shortnum:
                shortneed=tempitem
                shortnum=tempdic[str(tempitem.width)+str(tempitem.index)]

        for tempitem in otherneed:
            if tempitem.length/tempdic2[str(tempitem.width)+str(tempitem.index)]<shortneed.length/shortnum:
                shortneed=tempitem
                shortnum=tempdic2[str(tempitem.width)+str(tempitem.index)]
        
    else:

        for tempitem in mainneed:
            if tempitem.no_limit==0 and tempitem.length/tempdic[str(tempitem.width)+str(tempitem.index)]<shortneed.length/shortnum:
                shortneed=tempitem
                shortnum=tempdic[str(tempitem.width)+str(tempitem.index)]

        for tempitem in otherneed:
            if tempitem.no_limit==0 and tempitem.length/tempdic2[str(tempitem.width)+str(tempitem.index)]<shortneed.length/shortnum:
                shortneed=tempitem
                shortnum=tempdic2[str(tempitem.width)+str(tempitem.index)]
    ...
    if need_satisfy(shortneed.length,roll.length*num,shortneed.left,shortneed.right,min_length,shortneed.no_limit):#母卷能全分配满足需求
        print('step 1:')
        tempflag=0
        for need in mainneed:
            if need_satisfy(need.length,roll.length*tempdic[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
                tempflag=1
                break

        for need in otherneed:
            if need_satisfy(need.length,roll.length*tempdic2[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
                tempflag=1
                break
    ...
    if roll_satisfy(roll.length,cut_length,min_length):#母卷能满足需求
        ...
        for need in mainneed:
            if need==shortneed:
                continue
            if need_satisfy(need.length,cut_length*tempdic[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
                print(need.length,need.lengthscale.left,need.lengthscale.right)
                tempflag=1
                break

        for need in otherneed:
            if need_satisfy(need.length,cut_length*tempdic2[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
                print(need.length,need.lengthscale.left,need.lengthscale.right)
                tempflag=1
                break
    ...
    for i in range(int(max(interval.left,min_length)),int(interval.right)+1):#母卷能满足需求
        if roll_satisfy(roll.length,i,min_length)==True:
            ...
            for need in mainneed:
                if need_satisfy(need.length,i*tempdic[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
                    tempflag=1
                    break

            for need in otherneed:
                if need_satisfy(need.length,i*tempdic2[str(need.width)+str(need.index)],need.left,need.right,min_length,need.no_limit)==False:
                    tempflag=1
                    break
...
def get_maxlist0(needs,sameneeds,fneed,cut_length,max_width,min_length):
    ...
    for need in needs:
        if need.width+curwidth<=max_width and need not in fneed:
            if need_satisfy(need.length,cut_length,need.left,need.right,min_length,need.no_limit):
                ...
                for x in sameneeds:
                    if x.width==need.width and need_satisfy(x.length,cut_length,x.left,x.right,min_length,x.no_limit) and x not in fneed:
...
def get_better_roll_2(rolllist,content_width,content_sym,sym,allproject,remain_min,min_length):
    print("剩余组合分配：")
    tempsum=sum(content_width)
    ...
    roll_left=rolllist[0].widthscale.left
    roll_right=rolllist[0].widthscale.right


    if roll_right<tempsum or (roll_left>tempsum and roll_left-tempsum>=remain_min):#宽度指和不在母卷可用宽度范围内
        return -1
...
def all_match(mainproject,otherproject,storage,prolist,allproject,para,min_length,plan,remain_min,sym,method,idlist,rollindex,sorted_storage,left_cut,record,step):#最后一步分配完所有主需求
    ...
    while 1:#对于每一个主需求
        ...
        mainproject=[x for x in mainproject if x not in fneed]

        if len(mainproject)==0:
            break

        if order_flag==0:
            if start_flag==1:
                if temptotal1!=total1:
                    temptotal1=total1
                elif temptotal2!=total2:
                    temptotal2=total2
                else:
                    break
            else:
        ...
        mainproject.sort(key=lambda x:x.length)

        print("order_flag:"+str(order_flag))

        if order_flag==1:
            mainneed=orderneed
            order_flag=0
        else:
            mainneed=mainproject[0]
        ...
        for j in range(len(storage)):
            ...
            if isleft==1:
                ...
                better_index=get_better_roll_2(roll_same_width,leftneed,leftsym,sym,allproject,remain_min,min_length)
            ...
            while 1:
                ...
                if isleft==1 and roll.id not in sorted_storage:#有剩余组合，且当前母卷无连续关系
                    ...
                    if inscale(tempsum,roll.widthscale) or (roll.widthscale.left>tempsum and roll.widthscale.left-tempsum<remain_min):
                        ...
                        for index in range(len(leftneed)):
                            if leftsym[index]==sym['项目'][0]:
                                for x in mainproject:
                                    if x.width==leftneed[index] and x not in fneed:
                                        orderneed=x
                                        order_flag=1
                                        break
                            if order_flag==1:
                ...
                if need_satisfy(mainneed.length,roll.length,mainneed.left,mainneed.right,min_length,mainneed.no_limit)==True:#母卷全分配
                ...
                if result!={}:#成功分配，换下一卷
                    ...
                    if result!={} and lastwidth==result['width'] and roll.id==last_roll.id:
                        left_cut=left_cut+1
                        print("left_cut 15")
                ...
                if tempflag==0:#没分配完
                    ...
                    for tempneed in templist:
                        if tempneed in mainproject:
                            order_flag=1
                            orderneed=tempneed
                            break
                ...
                if result2!={}:#成功分配
                    ...
                    if result2!={} and lastwidth==result2['width'] and last_roll.id==roll.id:
                        left_cut=left_cut+1
                        print("left_cut 16")
                    ...
                    if result3!={}:#成功分配
                        ...
                        if result3!={} and lastwidth==result3['width'] and roll.id==last_roll.id:
                            left_cut=left_cut+1
                            print("left_cut 17")
